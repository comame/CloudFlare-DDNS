...
async function main() {
    ...
    const currentRecords = (await requestCloudflare(`zones/${zoneId}/dns_records`, 'get')).result

    const targetRecord = currentRecords.find(r =>
        r.name === domain &&
        r.type === 'A'
    )
...
async function requestCloudflare(url, method, body = null) {
    return new Promise((resolve, reject) => {
        ...
        const req = https.request('https://api.cloudflare.com/client/v4/' + url, {
            method: method,
            headers: {
